...
# Google Drive 파일 ID
file_id = '1-4jPafNwC7VGsdrBNqSxISlB763CtO-5'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/w60StfJ/dx.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=P7o0bNUGif8", start_time=0)
